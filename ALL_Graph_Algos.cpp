...
bool cycleDetectionDFSUni(vector<int>adj[],vector<int>&pervis,vector<int>vis,int node)
{
	vis[node]=1;
	pervis[node]=1;
	for(auto it:adj[node])
	{
		if(!vis[it])
		{
			if(!cycleDetectionDFSUni(adj,pervis,vis,node)) return true;
		}
		else if(pervis[it]==1) return true;
